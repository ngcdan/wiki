...
### `settings_location_state` — Tỉnh / Bang
...
| `country_id` | bigint | | FK quốc gia | `settings_country.id` |
| `country_code` | varchar(2) | | Mã quốc gia (denormalized) | |
...
### `settings_location_district` — Huyện / Quận
...
| `state_id` | bigint | | FK tỉnh/bang | `settings_location_state.id` |
| `state_label` | varchar | | Tên tỉnh/bang (denormalized) | |
| `gov_code` | varchar | | Mã hành chính nhà nước | |
...
### `settings_location_subdistrict` — Phường / Xã
...
| `district_id` | bigint | | FK huyện/quận | `settings_location_district.id` |
| `district_label` | varchar | | Tên huyện/quận (denormalized) | |
| `state_id` | bigint | | FK tỉnh/bang | `settings_location_state.id` |
| `state_label` | varchar | | Tên tỉnh/bang (denormalized) | |
| `gov_code` | varchar | | Mã hành chính nhà nước | |
...
| Trường | Kiểu | Bắt buộc | Mô tả | FK |
|---|---|---|---|---|
| `account_id` | bigint | | Account người phụ trách | `of1_fms_user_role.id` |
| `sale_owner_account_id` | bigint | | Account sale phụ trách | `of1_fms_user_role.id` |
| `sale_owner_full_name` | varchar | | Tên sale phụ trách | |
| `approved_by_account_id` | bigint | | Account duyệt | `of1_fms_user_role.id` |
| `approved_by_full_name` | varchar | | Tên người duyệt | |
| `request_by_account_id` | bigint | | Account yêu cầu tạo | `of1_fms_user_role.id` |
...
> **Denormalization note:** Bảng này intentionally lưu cả FK ID và label pairs (e.g., `country_id` + `country_label`) để tối ưu query performance. CDC handlers cần đảm bảo đồng bộ cả hai fields khi có thay đổi.
