...
## 2026-04-02: Update Debezium Connector — Password + Table List

### Thay doi

- Doi password MSSQL: `sa/12345678`
- Mo rong `table.include.list` tu 4 tables thanh 11 tables:
  `dbo.Partners, dbo.Transactions, dbo.HAWB, dbo.HAWBRATE, dbo.TransactionDetails, dbo.HAWBDETAILS, dbo.ContainerListOnHBL, dbo.SellingRate, dbo.BuyingRateWithHBL, dbo.OtherChargeDetail, dbo.ExchangeRate`

### Cach update Debezium connector

**Endpoint:** `http://debezium-connect.of1-dev-kafka.svc.cluster.local:8083`

**Config file:** `scripts/debezium-hps-connector.json`

**Buoc 1 — Sua config file** (`scripts/debezium-hps-connector.json`):
- Update `database.password`, `table.include.list`, hoac bat ky field nao can thay doi

**Buoc 2 — Xoa connector cu:**
```bash
curl -X DELETE http://debezium-connect.of1-dev-kafka.svc.cluster.local:8083/connectors/mssql-dev-hps-connector
```

**Buoc 3 — Tao lai connector voi config moi:**
```bash
...
**Luu y khi them table moi:**
- Phai enable CDC tren MSSQL truoc:
  ```sql
  EXEC sys.sp_cdc_enable_table @source_schema = N'dbo', @source_name = N'<TableName>', @role_name = NULL;
  ```
- Sau khi connector RUNNING, topic moi se duoc tao tu dong: `cdc-dev-hps.HPS_TEST_DB.dbo.<TableName>`
- Neu FMS server dang chay, can restart de consumer subscribe topic moi
