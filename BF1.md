...
RESTORE FILELISTONLY FROM DISK = N'C:\Backup\BF1.bak';

-- Restore database
RESTORE DATABASE [BF1]
FROM DISK = N'C:\Backup\BF1.bak'
WITH
  MOVE N'BF1_Data' TO N'C:\SQLData\BF1.mdf',
  MOVE N'BF1_Log'  TO N'C:\SQLData\BF1_log.ldf',
  REPLACE,
  STATS = 10;

-- Kiểm tra
SELECT name, state_desc FROM sys.databases WHERE name = 'BF1';
```
...
BACKUP DATABASE [BF1]
TO DISK = N'C:\Backup\BF1_full_20260317.bak'
WITH
  FORMAT,
  INIT,
...
-- Kiểm tra backup
RESTORE HEADERONLY FROM DISK = N'C:\Backup\BF1_full_20260317.bak';
```
...
sqlcmd -S localhost -U sa -P "PASSWORD" -Q "BACKUP DATABASE [BF1] TO DISK = N'C:\Backup\BF1_full.bak' WITH FORMAT, COMPRESSION, STATS = 10"
...
docker run --rm -it mcr.microsoft.com/mssql-tools \
  ...
  -Q "BACKUP DATABASE [BF1] TO DISK = N'C:\Backup\BF1_full.bak' WITH FORMAT, COMPRESSION"
```
...
bcp "SELECT * FROM BF1.dbo.TEN_BANG" queryout "C:\Export\TEN_BANG.csv" \
...
foreach ($t in $tables) {
  $t = $t.Trim()
  if ($t) {
    bcp "BF1.dbo.$t" out "C:\Export\$t.csv" -S localhost -U sa -P "PASSWORD" -c -t "," -r "\n"
  }
}
```
...
RESTORE FILELISTONLY FROM DISK = N'C:\Backup\BF1_full_20260317.bak';

-- Restore thành database mới (không ghi đè DB gốc)
RESTORE DATABASE [BF1_DEV]
FROM DISK = N'C:\Backup\BF1_full_20260317.bak'
WITH
  MOVE N'BF1_Data' TO N'C:\SQLData\BF1_DEV.mdf',
  MOVE N'BF1_Log'  TO N'C:\SQLData\BF1_DEV_log.ldf',
  REPLACE,
  STATS = 10;
```
...
sqlcmd -S SERVER_A -U sa -P "PASSWORD" -Q "BACKUP DATABASE [BF1] TO DISK = N'\\share\BF1.bak' WITH COMPRESSION"
sqlcmd -S SERVER_B -U sa -P "PASSWORD" -Q "RESTORE DATABASE [BF1] FROM DISK = N'\\share\BF1.bak' WITH MOVE N'BF1_Data' TO N'D:\Data\BF1.mdf', MOVE N'BF1_Log' TO N'D:\Data\BF1_log.ldf', REPLACE"
```
...
- Phân loại bảng theo mức ưu tiên:
  - **P0 — Bảng master/danh mục:** ít thay đổi, cần có đầu tiên để hiển thị được giao diện (VD: SDONVI, SCUAKHAU, SNGAN_HANG, ...).
  - **P1 — Bảng nghiệp vụ chính:** thay đổi thường xuyên, cần sync realtime (VD: DTOKHAIMD, DHANGMDDK, ...).
...
SELECT name, is_cdc_enabled FROM sys.databases WHERE name = 'BF1';
...
```json
{
  ...
  "config": {
    ...
    "database.hostname": "HOST_SQL_SERVER",
    "database.port": "1433",
    "database.user": "debezium",
    "database.password": "THAY_PASSWORD",
    "database.names": "BF1",
    "database.applicationName": "Debezium-BF1",

    "topic.prefix": "cdc-bf1",
    "table.include.list": "dbo.BANG_1,dbo.BANG_2,dbo.BANG_3",
...
sqlcmd -S HOST -U sa -P "PASSWORD" -d BF1 \
  -Q "UPDATE dbo.TEN_BANG SET COLUMN = COLUMN WHERE ID = 1"
...
-- Ví dụ tạo bảng (chuẩn hóa naming sang snake_case)
CREATE TABLE bf1.don_vi (
...
INSERT INTO bf1._table_mapping VALUES
  ('SDONVI', 'bf1.don_vi', 'Danh mục đơn vị'),
  ('DTOKHAIMD', 'bf1.to_khai_md', 'Tờ khai mở đầu');
```
...
-- Trên MSSQL:
SELECT 'SDONVI' AS tbl, COUNT(*) AS cnt FROM BF1.dbo.SDONVI
UNION ALL
SELECT 'DTOKHAIMD', COUNT(*) FROM BF1.dbo.DTOKHAIMD;

-- Trên PostgreSQL:
SELECT 'don_vi' AS tbl, COUNT(*) AS cnt FROM bf1.don_vi
UNION ALL
SELECT 'to_khai_md', COUNT(*) FROM bf1.to_khai_md;
```
